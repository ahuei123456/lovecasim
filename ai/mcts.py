...
class MCTSConfig:
    ...
    num_simulations: int = 100  # Number of simulations per move
    ...
    temperature: float = 1.0  # For action selection (low = greedy)
...
def play_game(mcts1: MCTS, mcts2: MCTS, verbose: bool = True) -> int:
    ...
    while not state.is_terminal() and move_count < max_moves:
        ...
        if verbose and move_count % 10 == 0:
            print(f"Move {move_count}: Player {state.current_player}, "
                  f"Phase {state.phase.name}, Action {action}")
...
def self_play(num_games: int = 10, simulations: int = 50) -> List[Tuple[List, List, int]]:
    ...
    for game_idx in range(num_games):
        ...
        print(f"Game {game_idx + 1}/{num_games} complete. "
              f"Moves: {move_count}, Winner: {winner}")
