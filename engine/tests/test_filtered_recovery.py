...
def game_state():
    ...
    state.member_db[400] = MemberCard(card_id=400, card_no="MUSE-01", name="Kotori", cost=3, group="μ's", hearts=np.zeros(6), blade_hearts=np.zeros(7), blades=1)
    # Member 401: μ's, Cost 5
    state.member_db[401] = MemberCard(card_id=401, card_no="MUSE-02", name="Honoka", cost=5, group="μ's", hearts=np.zeros(6), blade_hearts=np.zeros(7), blades=1)
    # Member 402: Aqours, Cost 2
    state.member_db[402] = MemberCard(card_id=402, card_no="AQ-01", name="Chika", cost=2, group="Aqours", hearts=np.zeros(6), blade_hearts=np.zeros(7), blades=1)
