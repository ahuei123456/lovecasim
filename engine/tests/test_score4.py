...
class TestScore4Abilities(unittest.TestCase):
    ...
    def test_honoka_blade_buff(self):
        ...
        self.state.live_db[1001] = LiveCard(1001, "L-1001", "S1", 1, np.zeros(7))
        self.state.live_db[1002] = LiveCard(1002, "L-1002", "S2", 1, np.zeros(7))
    ...
    def test_nico_score_bonus(self):
        ...
        for i in range(24):
            self.state.member_db[i] = MemberCard(i, f"TEST-{i}", f"Card{i}", 1, np.zeros(6), np.zeros(7), 1, group="ラブライブ！")
        ...
        self.state.member_db[24] = MemberCard(24, "TEST-24", "Card24", 1, np.zeros(6), np.zeros(7), 1, group="ラブライブ！")
