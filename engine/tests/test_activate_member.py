...
class TestActivateMember(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.state.member_db[100] = MemberCard(card_id=100, card_no="TEST-01", name="MyMem0", cost=1, hearts=np.zeros(6), blade_hearts=np.zeros(7), blades=1)
        self.state.member_db[101] = MemberCard(card_id=101, card_no="TEST-02", name="MyMem1", cost=1, hearts=np.zeros(6), blade_hearts=np.zeros(7), blades=1)
