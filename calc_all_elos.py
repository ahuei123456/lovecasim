...
for iteration in range(5):  # Iterate to converge
    ...
    for agent, data in stats.items():
        ...
        opponent_elos = [stats[opp]['current_elo'] if iteration == 0 else expected.get(opp, stats[opp]['current_elo']) 
                        for opp in stats if opp != agent]
...
for agent, elo in expected.items():
    ...
    print(f"{agent:12}: {elo:7.0f} ELO (should be), currently {1064 if agent=='Smart' else 975 if agent=='TrueRandom' else 959}")
