...
def run_single_game_stress(game_idx):
    ...
    try:
        ...
        for p in state.players:
            ...
            if not member_ids: raise Exception("No members found info DB")
    ...
    with Pool(cpu_count(), initializer=init_worker) as pool:
        for i, res in enumerate(pool.imap_unordered(run_wrapper, arg_generator())):
            ...
            if (i+1) % 5 == 0:
                ...
                print(f"Progress: {i+1} games | Crashes: {stats['crashes']} | Speed: {speed:.1f} games/s | Time: {elapsed:.1f}s", end='\r')
