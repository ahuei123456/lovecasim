...
from game.game_state import GameState, Phase, PlayerState, MemberCard
from game.data_loader import CardDataLoader
from game.ability import EffectType, Ability, TriggerType, Effect, TargetType
...
def test_combined_effects():
    ...
    complex_ability = Ability(
        ...
        effects=[
            Effect(EffectType.ORDER_DECK, 2, params={'position': 'bottom', 'shuffle': True}), # Shuffle top 2, put on bottom
    ...
    if len(p.hand) == 1:
        ...
        print(f"FAILURE: Draw NOT resolved. Hand size: {len(p.hand)}. Pending Effects: {[e.effect_type.name for e in state.pending_effects]}")
