...
def run_match(agent_a_name, agent_b_name, agents_map, num_games=10):
    ...
    for i in range(num_games):
        try:
            ...
            if winner == 0:
                p0_wins += 1
                if first_player == 0: first_player_wins += 1
            elif winner == 1:
                p1_wins += 1
                if first_player == 1: first_player_wins += 1
...
def main():
            ...
            with contextlib.redirect_stdout(io.StringIO()):
                ...
                for turn in range(5000):
                    ...
                    if turn > 4000:
                        ...
                        if turn == 4001:
                            print(f"  [WARN] Game {game_num} ({a} vs {b}) likely infinite loop. Actions sample:", file=sys.__stderr__)
                        if turn > 4990:
                            p_name = a if actions_this_game[-1][0] == 0 else b
                            print(f"    Action {turn}: Player {p_name} ({actions_this_game[-1][0]}) in {actions_this_game[-1][1]} phase did {action}", file=sys.__stderr__)
    ...
    with open("final_stats.txt", "w") as f:
        f.write(f"{'Agent':<15} | {'ELO':<6} | {'Games':<5} | {'Win Rate':<12} | {'True Wins':<10} | {'Draws':<6} | {'Avg Turns'}\n")
        f.write("-" * 95 + "\n")
        for agent_name in sorted(elo.ratings.keys(), key=lambda x: elo.ratings[x], reverse=True):
            elo_score = int(elo.ratings[agent_name])
            games = elo.matches[agent_name]
            ...
            avg_turns = f"{elo.total_turns[agent_name]/games:.1f}" if games > 0 else "N/A"
            f.write(f"{agent_name:<15} | {elo_score:<6} | {games:<5} | {win_rate:<12} | {true_wins:<10} | {draws:<6} | {avg_turns}\n")
        f.write("="*40 + "\n")
