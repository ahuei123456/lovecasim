...
if __name__ == "__main__":
    ...
    with Pool(cpu_count()) as pool:
        for i, result in enumerate(pool.imap_unordered(run_single_game, games_to_run)):
            ...
            if (i + 1) % 50 == 0:
                ...
                print(f"[{i+1}/{max_games}] Best Yells: {best['yells']} (Game #{best['id']+1}), Speed: {(i+1)/elapsed:.1f} games/s")
    ...
    for idx, game in enumerate(top_games):
        print(f"{idx+1}. Game #{game['id']+1}: {game['yells']} Yells, Score {game['p0_score']}-{game['p1_score']}, Winner: P{game['winner']}")
