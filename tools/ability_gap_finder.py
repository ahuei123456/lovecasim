    ...
    for card_no, data in cards.items():
        ...
        if not abilities:
            gaps.append({
                'no': card_no,
                'text': text,
                'reason': 'Parsing Failure (No abilities extracted)'
            })
            continue
        ...
        if all_meta:
            gaps.append({
                'no': card_no,
                'text': text,
                'reason': 'Meta-Rule Only (Engine logic likely missing)'
            })
            continue
        ...
        if found_keywords:
            gaps.append({
                'no': card_no,
                'text': text,
                'reason': f'Complex Keyword Match: {", ".join(found_keywords)}'
            })
...
if __name__ == "__main__":
    ...
    for g in gaps:
        reason = g['reason']
        if reason not in report: report[reason] = []
