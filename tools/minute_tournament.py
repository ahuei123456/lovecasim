...
def main():
    ...
    parser.add_argument("--duration", type=int, default=60, help="Tournament duration in seconds")
    ...
    with Pool(processes=num_workers, initializer=init_worker, initargs=(m, l)) as pool:
        ...
        while time.time() - start_time < args.duration:
            ...
            print(f"\r  Elapsed: {elapsed:.1f}s | Games: {total_games} | Rate: {total_games/elapsed:.1f} g/s", end="", flush=True)
    ...
    print("-" * 60)
    for name in sorted(G_AGENT_NAMES, key=lambda x: (agent_stats[x]["wins"]/(agent_stats[x]["matches"] or 1)), reverse=True):
