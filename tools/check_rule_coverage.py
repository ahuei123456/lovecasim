...
def check_coverage(rules_file, target_file):
    ...
    return sorted(found, key=lambda x: [int(y) for y in x.split('.')]), sorted(missing, key=lambda x: [int(y) for y in x.split('.')])
...
if os.path.exists(rules_path) and os.path.exists(target_path):
    ...
    print(f"Error: Files not found. rules exists: {os.path.exists(rules_path)}, target exists: {os.path.exists(target_path)}")
