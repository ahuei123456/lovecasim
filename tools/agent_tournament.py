...
def main():
    ...
    elo = EloRating()
    for name in agent_names: elo.init_agent(name)
    ...
    for result, (name_a, name_b) in zip(results, task_metadata):
        ...
        if result == 1.0: matchups[(name_a, name_b)][0] += 1
        elif result == 0.0: matchups[(name_a, name_b)][1] += 1
        else: matchups[(name_a, name_b)][2] += 1
