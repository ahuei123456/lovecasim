...
def mock_loader():
    ...
    m = MemberCard(
        card_id=1, name="Test Member", cost=1, hearts=np.array([1,0,0,0,0,0]), 
        blade_hearts=np.array([0,0,0,0,0,0]), blades=1, img_path="test.png"
    )
    m_db[1] = m
    
    # Create a live card
    l = LiveCard(
        card_id=100, name="Test Live", score=10, 
        required_hearts=np.array([1,0,0,0,0,0,0]), img_path="live.png"
    )
    l_db[100] = l
