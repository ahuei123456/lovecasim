...
def analyze_all_cards():
    ...
    for card_id, card_data in cards.items():
        ...
        has_ability = bool(card_data.get('ability'))
...
def generate_report(results):
    ...
    output.append(f"Generated: {len(results['with_ability']) + len(results['with_faq']) + len(results['vanilla'])} total cards analyzed\n")
    ...
    for card in sorted(results['with_ability'], key=lambda x: x['id']):
        ...
        output.append(f"| {card['id']} | {card['name']} | {card['type']} | {triggers} | {effects} | {conditions} | {costs} |\n")
