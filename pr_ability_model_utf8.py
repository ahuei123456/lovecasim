...
class TriggerType(IntEnum):
    NONE = 0
    ON_PLAY = 1  # 逋ｻ蝣ｴ譎・    ON_LIVE_START = 2  # 繝ｩ繧､繝夜幕蟋区凾
    ON_LIVE_SUCCESS = 3  # 繝ｩ繧､繝匁・蜉滓凾
    TURN_START = 4
    TURN_END = 5
    CONSTANT = 6  # 蟶ｸ譎・    ACTIVATED = 7  # 襍ｷ蜍・    ON_LEAVES = 8  # 閾ｪ蜍・- when member leaves stage/is discarded
...
class EffectType(IntEnum):
    ...
    REVEAL_CARDS = 26  # 蜈ｬ髢・- reveal cards from zone
    LOOK_AND_CHOOSE = 27  # 隕九ｋ縲√◎縺ｮ荳ｭ縺九ｉ - look at cards, choose from them
    CHEER_REVEAL = 28  # 繧ｨ繝ｼ繝ｫ縺ｫ繧医ｊ蜈ｬ髢・- cards revealed via cheer mechanic
    ACTIVATE_MEMBER = 29  # 繧｢繧ｯ繝・ぅ繝悶↓縺吶ｋ - untap/make active a member
    ADD_TO_HAND = 30  # 謇区惆縺ｫ蜉縺医ｋ - add card to hand (from any zone)
    COLOR_SELECT = 31  # Specify a heart color
    REPLACE_EFFECT = 34  # Replacement effect (莉｣繧上ｊ縺ｫ)
...
class ConditionType(IntEnum):
    ...
    COUNT_ENERGY = 14  # 繧ｨ繝阪Ν繧ｮ繝ｼ縺傾譫壻ｻ･荳・    HAS_LIVE_CARD = 15  # 繝ｩ繧､繝悶き繝ｼ繝峨′縺ゅｋ蝣ｴ蜷・    COST_CHECK = 16  # 繧ｳ繧ｹ繝医′X莉･荳・莉･荳・    RARITY_CHECK = 17  # Rarity filter
    HAND_HAS_NO_LIVE = 18  # Hand contains no live cards (usually paired with reveal cost)
    COUNT_SUCCESS_LIVE = 19  # 謌仙粥繝ｩ繧､繝悶き繝ｼ繝臥ｽｮ縺榊ｴ縺ｫX譫壻ｻ･荳・    OPPONENT_HAND_DIFF = 20  # Opponent has more/less/diff cards in hand
...
class Effect:
    ...
    params: Dict[str, Any] = field(default_factory=dict)
    is_optional: bool = False  # ・槭※繧ゅｈ縺・

class AbilityCostType(IntEnum):
    NONE = 0
    ENERGY = 1
    TAP_SELF = 2  # 繧ｦ繧ｧ繧､繝医↓縺吶ｋ
    DISCARD_HAND = 3  # 謇区惆繧呈昏縺ｦ繧・    RETURN_HAND = 4  # 謇区惆縺ｫ謌ｻ縺・(Self bounce)
    SACRIFICE_SELF = 5  # 縺薙・繝｡繝ｳ繝舌・繧呈而縺亥ｮ､縺ｫ鄂ｮ縺・    REVEAL_HAND_ALL = 6  # 謇区惆繧偵☆縺ｹ縺ｦ蜈ｬ髢九☆繧・    SACRIFICE_UNDER = 7  # 荳九↓鄂ｮ縺九ｌ縺ｦ縺・ｋ繧ｫ繝ｼ繝峨ｒ謗ｧ縺亥ｮ､縺ｫ鄂ｮ縺・    DISCARD_ENERGY = 8  # 繧ｨ繝阪Ν繧ｮ繝ｼ繧呈而縺亥ｮ､縺ｫ鄂ｮ縺・
