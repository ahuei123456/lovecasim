...
def validate_card(card):
    ...
    if count_match:
        ...
        has_count_condition = any(
            c.params.get('count') == expected_count 
            for c in all_conditions if hasattr(c, 'params') and c.params
        )
    ...
    if '相手' in text and ('手札' in text or 'メンバー' in text or 'デッキ' in text):
        has_opponent_effect = any(
            ...
            for e in all_effects if hasattr(e, 'params') and e.params
        )
    ...
    if len(found_zones) > 1:
        ...
        has_zone_param = any(
            c.params.get('zone') for c in all_conditions if hasattr(c, 'params') and c.params
        )
...
def extract_keywords(text):
    ...
    verbs = re.findall(r'([ぁ-んァ-ン一-龥]+)(する|できる|させる|なる|置く|引く|見る|選ぶ|得る|失う|移動|戻す|加える|払う|消す|与える|受ける|持つ|使う|出す|入れる|送る|渡す|変える|増える|減る|残す|捨てる|探す|公開|確認|宣言|聞く)', text)
    for v in verbs:
...
def analyze_keyword_gaps(cards):
    ...
    KNOWN_HANDLED = {
        # Effects
        '引く', '得る', '置く', '戻す', '加える', '移動', '探す', '見る', '選ぶ',
        'アクティブ', 'ウェイト', 'バトンタッチ', 'スコア', 'ブレード', 'ハート',
        'エネルギー', 'コスト', 'デッキ', '手札', '控え室', 'ステージ', 'エリア',
        # Triggers
        '登場時', 'ライブ成功時', 'ライブ開始時', 'ターン開始', 'ターン終了', '常時', '起動',
        # Conditions
        '場合', '以上', '以下', '限り', '時', 'のみ',
        # Targets
        'メンバー', 'カード', '自分', '相手', '全員',
        # Modifiers
        'ターン1回', 'ライブ終了時まで', 'ターン終了時まで',
        # Groups/Characters (handled generically)
        "μ's", 'Aqours', 'Liella!', '虹ヶ咲', '蓮ノ空',
    }
